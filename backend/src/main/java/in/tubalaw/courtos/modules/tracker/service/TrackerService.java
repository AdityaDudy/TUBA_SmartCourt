...
public class TrackerService {
    ...
    public void diffAndNotify(String cnr, String tenantId, int newHearings, int newOrders) {
        ...
        if (tc == null || tc.getMatterId() == null)
            return;
    ...
    private CaseDetailResponse toDetailResponse(TrackedCase tc, String tenantId, Long userId) {
        ...
        if ((judgeName == null || judgeName.isBlank()) && !hearings.isEmpty()) {
            for (CaseHearing ch : hearings) {
                if (ch.getJudge() != null && !ch.getJudge().isBlank()) {
                    judgeName = ch.getJudge();
                    break;
                }
            }
        }
        ...
        String courtName = tc.getCourtName() != null ? tc.getCourtName() : (tc.getCnr() != null && tc.getCnr().startsWith("DLHC") ? "High Court of Delhi, Delhi" : null);
        String courtComplex = tc.getCourtComplex() != null ? tc.getCourtComplex() : (tc.getCnr() != null && tc.getCnr().startsWith("DLHC") ? "High Court of Delhi" : null);
    ...
    private void mapResultToEntity(CaseDetailResponse r, TrackedCase tc) {
        if (r.getCaseType() != null) tc.setCaseType(r.getCaseType());
        if (r.getFilingNo() != null) tc.setFilingNo(r.getFilingNo());
        if (r.getFilingDate() != null) tc.setFilingDate(tryParseDate(r.getFilingDate()));
        if (r.getRegistrationNo() != null) tc.setRegistrationNo(r.getRegistrationNo());
        if (r.getRegistrationDate() != null) tc.setRegistrationDate(tryParseDate(r.getRegistrationDate()));
        if (r.getCourtName() != null) tc.setCourtName(r.getCourtName());
        if (r.getCourtComplex() != null) tc.setCourtComplex(r.getCourtComplex());
        if (r.getJudgeName() != null && !r.getJudgeName().isBlank()) tc.setJudgeName(r.getJudgeName());
        if (r.getCaseStatus() != null) tc.setCaseStatus(r.getCaseStatus());
        if (r.getStageOfCase() != null) tc.setStageOfCase(r.getStageOfCase());
        if (r.getNextHearingDate() != null) tc.setNextHearingDate(tryParseDate(r.getNextHearingDate()));
        if (r.getFirNo() != null) tc.setFirNo(r.getFirNo());
        if (r.getFirYear() != null) tc.setFirYear(r.getFirYear());
        if (r.getPoliceStation() != null) tc.setPoliceStation(r.getPoliceStation());
