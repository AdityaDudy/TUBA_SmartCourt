...
public class FilingController {
    ...
    public ResponseEntity<ApiResponse<List<Filing>>> list(@RequestParam(required = false) String status) {
        ...
        if (scope == SecurityScopeContext.DataScope.ORG) {
            list = status != null ? repo.findAllByTenantIdAndStatus(TENANT, status)
                    : repo.findAllByTenantId(TENANT);
        } else {
            String userName = user != null && user.getEmail() != null ? user.getEmail().split("@")[0].replace(".", " ") : "";
            list = status != null ? repo.findAllByTenantIdAndStatusAndAdvocateContaining(TENANT, status, userName)
                    : repo.findAllByTenantIdAndAdvocateContaining(TENANT, userName);
        }
