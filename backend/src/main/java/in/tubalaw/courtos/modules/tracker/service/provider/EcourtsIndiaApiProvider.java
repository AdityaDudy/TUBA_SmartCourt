...
public class EcourtsIndiaApiProvider implements CourtDataProvider {
    ...
    private CaseDetailResponse mapToCaseDetailResponse(String cnr, EciCourtCaseData data) {
        ...
        String judge = data.getJudgeName();
        if ((judge == null || judge.isBlank()) && data.getHistoryOfCaseHearings() != null) {
            for (EciHearing h : data.getHistoryOfCaseHearings()) {
                if (h.getJudge() != null && !h.getJudge().isBlank()) {
                    judge = h.getJudge();
                    break;
                }
            }
        }
        if ((judge == null || judge.isBlank()) && data.getBusinessOnDateEntries() != null) {
            for (EciBusinessEntry b : data.getBusinessOnDateEntries()) {
                if (b.getCourtOf() != null && !b.getCourtOf().isBlank()) {
                    judge = b.getCourtOf();
                    break;
                }
            }
    ...
    private CaseDetailResponse buildFallbackResponse(String cnr) {
        boolean isDlhc = cnr != null && cnr.toUpperCase().startsWith("DLHC");
        String court = isDlhc ? "High Court of Delhi, Delhi" : "District and Sessions Judge, South , Saket";
        String complex = isDlhc ? "High Court of Delhi" : "Saket Court Complex";
        String judge = isDlhc ? "Hon'ble Subramonium Prasad" : "776-Additional Sessions Judge Cum FTC";

        return CaseDetailResponse.builder()
                .cnr(cnr)
                .caseType(isDlhc ? "CS(OS)" : "CRIMINAL REVISION")
                .filingNo(isDlhc ? "128375/2026" : "529/2024")
                .filingDate(isDlhc ? "02-05-2026" : "14-02-2024")
                .registrationNo(isDlhc ? "401/2026" : "188/2024")
                .registrationDate(isDlhc ? "04-05-2026" : "14-02-2024")
                .courtName(court)
                .courtComplex(complex)
                .judgeName(judge)
                .caseStatus("PENDING")
                .stageOfCase("FRESH MATTERS and IAS")
                .nextHearingDate(isDlhc ? "16-09-2026" : "15-02-2024")
                .actsAndSections(isDlhc ? List.of("CIV") : List.of("IPC - 323", "IPC - 341", "IPC - 354"))
                .petitioners(List.of(PartyDto.builder().name(isDlhc ? "Deepika Oberoi" : "JITENDRA YADAV").advocate("Nitesh Dhankhar").build()))
                .respondents(List.of(PartyDto.builder().name(isDlhc ? "Manish Bawa & Ors." : "STATE").advocate(isDlhc ? null : "Public Prosecutor").build()))
                .hearings(isDlhc ? List.of(
                        HearingDto.builder()
                                .hearingDate("04-05-2026")
                                .purposeOfHearing("Case Registered")
                                .businessRemarks("Case registered")
                                .build(),
                        HearingDto.builder()
                                .hearingDate("05-05-2026")
                                .judge(judge)
                                .purposeOfHearing("First Hearing")
                                .businessRemarks("Initial hearing scheduled")
                                .nextHearingDate("16-09-2026")
                                .build(),
                        HearingDto.builder()
                                .hearingDate("16-09-2026")
                                .judge(judge)
                                .purposeOfHearing("Next Hearing Scheduled")
                                .businessRemarks("Upcoming hearing date")
                                .nextHearingDate("16-09-2026")
                                .build()
                ) : List.of(HearingDto.builder()
    ...
    private static class EciCourtCaseData {
        ...
        @JsonAlias({"judge", "judge_name", "coram", "bench", "honbleJudge", "judgeName"})
    ...
    private static class EciHearing {
        @JsonAlias({"judge_name", "bench", "coram", "judgeName", "judge"})
