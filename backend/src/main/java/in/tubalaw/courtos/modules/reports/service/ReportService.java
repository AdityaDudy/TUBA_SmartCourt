...
public class ReportService {
    ...
    public Map<String, Object> getSummary(String fy, String practiceArea, String court, String advocate) {
        ...
        if (fy != null && !fy.trim().isEmpty() && !"all".equalsIgnoreCase(fy.trim())) {
            try {
        ...
        List<Matter> filteredMatters = matters.stream()
            .filter(m -> {
                if (finalEnd != null) {
                    ...
                    if (fd != null && fd.isAfter(finalEnd)) return false;
                }
                return true;
            })
        ...
        double collectedRevenue = filteredInvoices.stream()
            ...
            .mapToDouble(inv -> {
                if (inv.getPaidAmount() != null && inv.getPaidAmount().doubleValue() > 0) {
                    return inv.getPaidAmount().doubleValue();
                }
                return inv.getAmount() != null ? inv.getAmount().doubleValue() : 0.0;
            })
            .sum();

        // ── 4. Clients Count ────────────────────────────────────────
        boolean hasSpecificFilters = (practiceArea != null && !practiceArea.trim().isEmpty()) ||
                                     (court != null && !court.trim().isEmpty()) ||
                                     (advocate != null && !advocate.trim().isEmpty());

        long activeClientsCount;
        if (hasSpecificFilters) {
            activeClientsCount = clients.stream()
                .filter(c -> filteredMatters.stream().anyMatch(m -> 
                    (m.getClientId() != null && m.getClientId().equals(c.getId())) ||
                    (m.getClientName() != null && c.getName() != null && m.getClientName().trim().equalsIgnoreCase(c.getName().trim()))
                ))
                .count();
        } else {
            activeClientsCount = clients.stream()
                .filter(c -> c.getStatus() == null || !"inactive".equalsIgnoreCase(c.getStatus()))
                .count();
        }
