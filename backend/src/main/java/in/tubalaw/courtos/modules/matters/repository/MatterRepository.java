...
public interface MatterRepository extends JpaRepository<Matter, Long>,
        JpaSpecificationExecutor<Matter> {

    // ── Dashboard aggregation ─────────────────────────────────────────────
    /**
     * Court distribution for DashboardController.courtDistribution().
     * Replaces the former findAllByTenantId() + Java groupBy, eliminating the full-scan.
     */
    @Query("SELECT m.court as court, COUNT(m) as count " +
           "FROM Matter m WHERE m.tenantId = :tenantId GROUP BY m.court")
    List<CourtCount> countByCourt(String tenantId);

    /** Projection used by countByCourt() */
