...
export class FilingsPageComponent implements OnInit {
  ...
  getKanbanFilings(stage: string) {
    return this.filtered.filter(f => {
      const stageLower = f.stage?.toLowerCase() || '';
      const targetLower = stage.toLowerCase();
      if (targetLower.includes('defect')) return stageLower.includes('defect');
      return stageLower === targetLower;
    });
  }

  // Per-column Kanban pagination signals & getters
  kanbanPageMap = signal<Record<string, number>>({});
  readonly kanbanSize = 5;

  getKanbanPage(stage: string): number {
    return this.kanbanPageMap()[stage] || 1;
  }

  setKanbanPage(stage: string, delta: number) {
    const total = this.getKanbanTotalPages(stage);
    const current = this.getKanbanPage(stage);
    const next = Math.max(1, Math.min(total, current + delta));
    this.kanbanPageMap.update(m => ({ ...m, [stage]: next }));
  }

  getKanbanTotalPages(stage: string): number {
    const total = this.getKanbanFilings(stage).length;
    return Math.max(1, Math.ceil(total / this.kanbanSize));
  }

  getPaginatedKanbanFilings(stage: string) {
    const list = this.getKanbanFilings(stage);
    const p = Math.min(this.getKanbanPage(stage), this.getKanbanTotalPages(stage));
    return list.slice((p - 1) * this.kanbanSize, p * this.kanbanSize);
  }
