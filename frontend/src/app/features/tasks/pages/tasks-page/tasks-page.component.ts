...
export class TasksPageComponent implements OnInit {
  ...
  private sortByLatest(list: Task[]): Task[] {
  ...
  get filtered() {
    let list = this.sortByLatest(this.tasks());
  ...
  // Per-column Kanban pagination signals & getters
  kanbanPageTodo = signal(1);
  kanbanPageOverdue = signal(1);
  kanbanPageUnderReview = signal(1);
  kanbanPageDone = signal(1);
  readonly kanbanSize = 5;

  get totalPagesTodo() { return Math.max(1, Math.ceil(this.todoTasks.length / this.kanbanSize)); }
  get totalPagesOverdue() { return Math.max(1, Math.ceil(this.overdueTasks.length / this.kanbanSize)); }
  get totalPagesUnderReview() { return Math.max(1, Math.ceil(this.underReviewTasks.length / this.kanbanSize)); }
  get totalPagesDone() { return Math.max(1, Math.ceil(this.doneTasks.length / this.kanbanSize)); }

  get paginatedTodoTasks() {
    const p = Math.min(this.kanbanPageTodo(), this.totalPagesTodo);
    return this.todoTasks.slice((p - 1) * this.kanbanSize, p * this.kanbanSize);
  }

  get paginatedOverdueTasks() {
    const p = Math.min(this.kanbanPageOverdue(), this.totalPagesOverdue);
    return this.overdueTasks.slice((p - 1) * this.kanbanSize, p * this.kanbanSize);
  }

  get paginatedUnderReviewTasks() {
    const p = Math.min(this.kanbanPageUnderReview(), this.totalPagesUnderReview);
    return this.underReviewTasks.slice((p - 1) * this.kanbanSize, p * this.kanbanSize);
  }

  get paginatedDoneTasks() {
    const p = Math.min(this.kanbanPageDone(), this.totalPagesDone);
    return this.doneTasks.slice((p - 1) * this.kanbanSize, p * this.kanbanSize);
  }

  // List View Pagination getters
  get totalPages() {
