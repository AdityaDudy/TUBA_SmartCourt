...
export class BillingPageComponent implements OnInit {
    ...
    return this.billing()
      ...
      .filter(b => {
        ...
        return (
          ...
          (b.matterTitle?.toLowerCase().includes(query))
        );
      });
  }
  ...
  get filteredExpenses() {
    return this.expenses().filter(e => {
      ...
      return (
        ...
        (e.matterTitle?.toLowerCase().includes(query)) ||
  ...
  paginatedExpenses = computed(() => {
    const start = (this.expensesPage() - 1) * this.expensesPageSize;
    return this.filteredExpenses.slice(start, start + this.expensesPageSize);
  });

  expenseTotalPages = computed(() => {
    return Math.ceil(this.filteredExpenses.length / this.expensesPageSize) || 1;
  });
  ...
  generateInvoiceFromPending() {
    ...
    this.form.set({
      ...
      matterTitle: first.matterTitle || 'Unlinked',
      dueDate: new Date(Date.now() + 15 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
  ...
  bulkExport() {
    const list = this.bulkSelectedInvoices();
    const count = list.length || this.filtered.length;
