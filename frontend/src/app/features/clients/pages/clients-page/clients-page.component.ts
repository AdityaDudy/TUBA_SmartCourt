...
export class ClientsPageComponent implements OnInit {
  ...
  getActiveMattersCount(clientId: any): number {
    ...
    return this.ds.matters().filter(m => String(m.clientId) === String(clientId) && m.status === 'Active').length;
  }

  get filtered() {
    return this.clients()
      .filter(c => this.filter() === 'All' || c.type === this.filter())
      .filter(c => !this.search() || c.name?.toLowerCase().includes(this.search().toLowerCase()));
  }
