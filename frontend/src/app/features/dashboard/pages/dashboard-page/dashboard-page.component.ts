...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, DatePipe, RouterLink, SkeletonComponent, EmptyStateComponent, CountUpDirective, AppButtonComponent],
...
export class DashboardPageComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.ds.filterHearings('All').subscribe(h => {
      this.monthHearings.set(h);
    });
  ...
  get todayHearings() {
    const list = this.monthHearings().length > 0 ? this.monthHearings() : this.hearings();
    const currentMonthPrefix = new Date().toISOString().slice(0, 7); // e.g. "2026-08"
    return list
      .filter(h => h.status !== 'Completed' && h.hearingDate && h.hearingDate.startsWith(currentMonthPrefix))
