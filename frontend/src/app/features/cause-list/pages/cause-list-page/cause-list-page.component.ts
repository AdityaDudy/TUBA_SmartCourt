...
export class CauseListPageComponent implements OnInit {
  ...
  filtered = computed<Hearing[]>(() => {
    ...
    return this.hearings()
      ...
      .sort((a, b) => {
        ...
        // Both are upcoming/future hearings: sort by nearest date first (ascending)
        if (isUpcomingA && isUpcomingB) {
          return dateA.localeCompare(dateB);
        }
        // Upcoming hearings always rank above past hearings
        if (isUpcomingA && !isUpcomingB) return -1;
        if (!isUpcomingA && isUpcomingB) return 1;

        // Both are past hearings: sort by most recent past date first (descending)
  ...
  setDateMode(mode: 'date' | 'all'): void {
  ...
  displayDate(): string {
    if (this.dateMode() === 'all') return 'All Hearings Overview';
