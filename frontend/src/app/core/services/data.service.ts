...
import { tap, catchError, map, finalize, debounceTime, switchMap, take } from 'rxjs/operators';
...
export class DataService {
  ...
  // Total counts from paginated responses (totalElements from server)
  readonly matterTotal  = computed(() => this._state().matters.total);
  readonly clientTotal  = computed(() => this._state().clients.total);
  readonly taskTotal    = computed(() => this._state().tasks.total);
  readonly filingTotal  = computed(() => this._state().filings.total);
  readonly billingTotal = computed(() => this._state().billing.total);
  ...
  constructor() {
    this.initSseStreams();
    // Debounced dashboard refresh triggered by mutations
    this._dashDirty$.pipe(debounceTime(1500)).subscribe(() =>
      this.loadDashboardStats().pipe(take(1)).subscribe()
    );
  }

  /** Schedules a debounced dashboard stats refresh (1.5 s). Use instead of inline subscribe(). */
  markDashDirty() { this._dashDirty$.next(); }
  ...
  // Dashboard dirty signal
  private readonly _dashDirty$ = new Subject<void>();
  ...
  /** HTTP GET that unwraps PagedApiResponse<T> (server-side paginated lists) */
  private getPaged<T>(url: string, params?: HttpParams): Observable<import('../api/api-response.types').PagedResponse<T>> {
    return this.http.get<import('../api/api-response.types').PagedResponse<T>>(url, { params }).pipe(
      catchError(err => this._handleError(err)),
    );
  }
  ...
  //  MATTERS
  // ============================================================
  loadMatters(params?: ListParams): Observable<import('../api/api-response.types').PagedResponse<Matter>> {
    this.patch('matters', { loading: true, error: null });
    const qp = this._toPagedParams(params);
    return this.getPaged<Matter>(API.MATTERS.LIST, qp).pipe(
      tap(page => this.patch('matters', { items: page.content, loaded: true, total: page.totalElements })),
  ...
  //  CLIENTS
  // ============================================================
  loadClients(params?: ListParams): Observable<import('../api/api-response.types').PagedResponse<Client>> {
    this.patch('clients', { loading: true, error: null });
    return this.getPaged<Client>(API.CLIENTS.LIST, this._toPagedParams(params)).pipe(
      tap(page => this.patch('clients', { items: page.content, loaded: true, total: page.totalElements })),
  ...
  //  TASKS
  // ============================================================
  loadTasks(params?: ListParams): Observable<import('../api/api-response.types').PagedResponse<Task>> {
    this.patch('tasks', { loading: true, error: null });
    return this.getPaged<Task>(API.TASKS.LIST, this._toPagedParams(params)).pipe(
      tap(page => this.patch('tasks', { items: page.content, loaded: true, total: page.totalElements })),
  ...
  //  FILINGS
  // ============================================================
  loadFilings(params?: ListParams): Observable<import('../api/api-response.types').PagedResponse<Filing>> {
    this.patch('filings', { loading: true, error: null });
    return this.getPaged<Filing>(API.FILINGS.LIST, this._toPagedParams(params)).pipe(
      tap(page => this.patch('filings', { items: page.content, loaded: true, total: page.totalElements })),
  ...
  //  BILLING & EXPENSES
  // ============================================================
  loadBilling(params?: ListParams): Observable<import('../api/api-response.types').PagedResponse<Invoice>> {
    this.patch('billing', { loading: true, error: null });
    return this.getPaged<Invoice>(API.BILLING.INVOICES, this._toPagedParams(params)).pipe(
      tap(page => this.patch('billing', { items: page.content, loaded: true, total: page.totalElements })),
  ...
  //  PRIVATE HELPERS
  // ============================================================
  /** Builds HttpParams from ListParams for paginated endpoints (includes page/size/sort/order/search + filter keys). */
  private _toPagedParams(params?: ListParams): HttpParams | undefined {
    ...
    if (params.filter) {
      for (const [k, v] of Object.entries(params.filter)) {
        if (v !== undefined && v !== null && v !== '') {
          p = p.set(k, Array.isArray(v) ? v.join(',') : v);
        }
      }
    }
    return p;
